...
def returnToMenu():
    ...
    root.unbind(
        "<Configure>",
        lambda event: iOFrame.place_configure(relx=0.5, rely=0.5, anchor="center"),
    )
...
# Navagation Bar
# ===============
navBar = Frame(master=root, bg=root["bg"], height=100, width=root.winfo_screenwidth())
...
# ==============================
iOFrame = Frame(master=root, bg=root["bg"], width=(root.winfo_screenwidth() - 100))
...
# =======================
titlePageFrame = Frame(master=root, bg=root["bg"])
titlePageFrame.pack()
titlePageFrame.place(relx=0.5, rely=0.5, anchor="center")
