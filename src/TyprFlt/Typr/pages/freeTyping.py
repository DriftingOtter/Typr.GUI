...
class FreeTyping(ft.UserControl):
    ...
    def initialize_banners(self):
        self.db_saver_banner_fail = self.create_banner(
            ...
            "Oops, could not save user data to database.",
            self.fail_close_banner,
        )
        self.db_saver_banner_pass = self.create_banner(
            ft.colors.GREEN,
            ft.icons.CHECK,
            "Great, successfully saved user data to database.",
            self.pass_close_banner,
        )
    ...
    def create_results_dialogue(self, results):
        title = ft.Text(
            "Here are your results \U0001F9D9", style=ft.TextThemeStyle.DISPLAY_LARGE
        )
        content = ft.Text(
    ...
    def reset_inputs(self):
        ...
        logging.info(
            "[COMPLETED] New Text Generated and inputs reset at %s.",
